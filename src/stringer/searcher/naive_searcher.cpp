...
Searcher::Indices NaiveSearcher::searchWildcards(const std::string& text, const std::string& pattern, size_t& char_comparisons, char wildcard)
{
    Searcher::Indices indices;
    ...
    for (size_t text_index = 0; text_index < text.size(); ++text_index) {
        ...
        for (size_t pattern_index = 0; pattern_index < pattern.size(); ++pattern_index) {
            if (text_index + pattern_index >= text.size()
                || !compareWildcards(text[text_index + pattern_index], pattern[pattern_index], char_comparisons, wildcard)) {
                found = false;
                break;
            }
        }
